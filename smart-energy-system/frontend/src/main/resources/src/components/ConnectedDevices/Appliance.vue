<template>
  <b-card :title="`Appliance (${id})`" class="border-0 bg-transparent">
    <b-card>
      <ul>
          <li v-for="(value, key) in item" :key="key">{{ key }}: {{ value }}</li>
      </ul>
      <p v-if="this.item.enabled">Appliance is currently <strong>enabled</strong>!</p>
      <p v-if="!this.item.enabled">Appliance is currently <strong>disabled</strong>!</p>
      <b-button
...
export default {
  ...
  mounted() {
    Axios.get(`${API_HOUSEHOLDS}/${this.household.id}/appliances/${this.id}`)
      .then((response) => {
